...
class AuthResource(AsyncResource):
    ...
    async def login(
        ...
        return self._store(self._model(await self._transport.request(req), AuthResponse))

    async def login_social(self, token: str, *, device_id: Optional[str] = None) -> AuthResponse:
    ...
    async def reset_password(self, email: str) -> Optional[ActionResult]:
        req = PreparedRequest("POST", "/auth/reset_password", json={"email": email})
        return self._model_opt(await self._transport.request(req), ActionResult)
...
class AccountResource(AsyncResource):
    ...
    async def get_profile(self) -> ProfileResponse:
        ...
        return self._model(await self._transport.request(req), ProfileResponse)
    ...
    async def update_email(self, email: str, password: str) -> ProfileResponse:
        req = PreparedRequest(
            "PATCH",
            "/users/settings",
    ...
    async def get_privacy(self) -> int:
    ...
    async def update_privacy(self, profile_feed: int) -> bool:
        req = PreparedRequest(
    ...
    async def deactivate(self, password: str, reason: str) -> bool:
    ...
    async def delete_device_token(self, device_id: str) -> bool:
        req = PreparedRequest(
            "DELETE", "/users/device_token", json={"device_id": device_id}
        )
        return self._bool(await self._transport.request(req))

    async def map_profile_picture(self, picture: str) -> bool:
    ...
    async def get_activities(self) -> ActivitiesResponse:
        ...
        return self._model(await self._transport.request(req), ActivitiesResponse)

    async def claim_reward(self, activity_id: Any, tier: Any) -> bool:
        req = PreparedRequest(
            "POST", f"/users/activities/{activity_id}/claim", params={"tier": tier}
        )
        return self._bool(await self._transport.request(req))

    async def get_coins(self) -> Optional[CoinResponse]:
        req = PreparedRequest("GET", "/payment/coins")
        return self._model_opt(await self._transport.request(req), CoinResponse)

    async def get_plans(self) -> list[Coin]:
        req = PreparedRequest("GET", "/payment/plans")
        return self._list(await self._transport.request(req), Coin)
...
class UsersResource(AsyncResource):
    """Other users and people pages."""

    async def get_user_info(self, user_id: Any) -> Optional[FriendProfile]:
        req = PreparedRequest("GET", f"/users/{user_id}")
        return self._model_opt(await self._transport.request(req), FriendProfile)

    async def get_person(self, person_id: Any) -> UserProfileResponse:
        req = PreparedRequest("GET", f"/people/{person_id}")
        return self._model(await self._transport.request(req), UserProfileResponse)

    async def get_person_credits(self, person_id: Any) -> CreditsResponse:
        req = PreparedRequest("GET", f"/people/{person_id}/credits")
        return self._model(await self._transport.request(req), CreditsResponse)

    async def like_person(self, person_id: Any, liked: bool = True) -> str:
        req = PreparedRequest(
            "POST", f"/people/{person_id}/likes", json={"liked": liked}
        )
        return self._str(await self._transport.request(req))

    async def get_user_stats(self, user_id: Any) -> PersonStats:
        req = PreparedRequest("GET", f"/users/{user_id}/stats")
        return self._model(await self._transport.request(req), PersonStats)

    async def get_user_watchlist(self, **params: Any) -> list[Movie]:
