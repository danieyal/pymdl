...
class SubscriptionResource(AsyncResource):
    """MySubscriptionRepository."""

    async def get(self) -> MySubscription:
        req = PreparedRequest("GET", "/subscriptions")
        return self._model(await self._transport.request(req), MySubscription)


class FlowersResource(AsyncResource):
    """SendFlowersRepository."""

    async def gift_process(self, person_id: Any) -> PersonGiftProcess:
        req = PreparedRequest("GET", f"/people/{person_id}/gift_process")
        return self._model(await self._transport.request(req), PersonGiftProcess)

    async def top_senders(self, person_id: Any) -> Any:
        req = PreparedRequest("GET", f"/people/{person_id}/gifts")
        return self._raw(await self._transport.request(req))

    async def send(self, person_id: Any, amount: int) -> bool:
        req = PreparedRequest(
...
class AwardsResource(AsyncResource):
    """Awards listing and giving (TitleRepository)."""

    async def list(self) -> AwardResponse:
        req = PreparedRequest("GET", "/awards")
        return self._model(await self._transport.request(req), AwardResponse)
...
class ReportsResource(AsyncResource):
    """Report rules and submission."""

    async def rules(self, *, type: str = "comment") -> Any:
...
class CalendarResource(AsyncResource):
    """EpisodesRepository + QuarterlyRepository."""

    async def episodes(self) -> Episodes:
        req = PreparedRequest("POST", "/calendar/episodes")
        return self._model(await self._transport.request(req), Episodes)

    async def quarter(self, *, year: int, quarter: Any) -> list[Quarter]:
        req = PreparedRequest(
            "POST", "/calendar/quarter", json={"year": year, "quarter": quarter}
        )
        return self._list(await self._transport.request(req), Quarter)


class LeaderboardResource(AsyncResource):
    """LeaderboardRepository."""

    async def get(self, time_period: Union[int, str] = "alltime") -> list[Entries]:
        ...
        return self._list(await self._transport.request(req), Entries, key="entries")
